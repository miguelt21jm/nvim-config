...
function telescopePickers.prettyDocumentSymbols(localOptions)
	...
	local opts = vim.tbl_deep_extend("force", {}, localOptions)

	local originalEntryMaker = telescopeMakeEntryModule.gen_from_lsp_symbols(opts)
	...
	opts.entry_maker = function(line)
		...
		local displayer = telescopeEntryDisplayModule.create({
			...
			items = {
				{ width = fileTypeIconWidth },
				{ width = 20 }, -- adjust if you need a wider “kind” column
				{ remaining = true },
			},
		})

		originalEntryTable.display = function(entry)
			local kind_key = entry.symbol_type:lower():gsub("^%l", string.upper)
			local icon = kind_icons[kind_key] or ""
			return displayer({
				icon,
